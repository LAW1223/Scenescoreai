...
export default function SeriesDetail() {
  ...
  const typeLabel = isTraditional
    ...
    : work.type
  const directorLabel = isTraditional && work.director === 'Director name' ? '導演姓名' : work.director
  ...
  return (
    <div className="detail-page" lang={isTraditional ? 'zh-Hant' : 'en'}>
      <section className={`detail-hero scene-poster scene-poster--${work.accent}`}>
        ...
        <div className="detail-hero__title"><span className="section-kicker">{copy.featured}</span><h1>{work.title}</h1><p>{directorLabel}</p></div>
      ...
      <section className="detail-overview page-pad">
        <div><span className="section-kicker">{copy.overview}</span><h2>{copy.overviewTitle}</h2></div>
      ...
      <section className="detail-next page-pad">
        <span className="section-kicker">{copy.next}</span>
      ...
      {crewOpen && (
        <motion.div className="crew-drawer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} role="dialog" aria-modal="true" aria-label="Crew information">
          ...
          <span className="section-kicker">/ {copy.crewInfo}</span><h2>{work.title}</h2><p>{copy.director} / {directorLabel}</p><p>{copy.production}</p>
        </motion.div>
      )}
